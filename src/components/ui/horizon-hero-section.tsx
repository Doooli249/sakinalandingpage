...
export const Component = () => {
  ...
  useEffect(() => {
    ...
    const createNebula = () => {
      ...
      const nebula = new THREE.Mesh(geometry, material) as MeshLike;
    ...
    const createMountains = () => {
      ...
      layers.forEach((layer, index) => {
        ...
        const mountain = new THREE.Mesh(geometry, material) as MeshLike;
    ...
    const animate = () => {
      ...
      if (refs.nebula) {
        ...
        if (material.uniforms?.time) material.uniforms.time.value = time * 0.5;
      }
  ...
  useEffect(() => {
    ...
    if (titleRef.current) {
      tl.from(titleRef.current, { y: 80, opacity: 0, duration: 1.2, ease: "power4.out" }, "-=0.4");
    }
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      refs.mountains.forEach((mountain, i) => {
        const speed = 1 + i * 0.9;
        const baseZ = (mountain.userData.baseZ as number) ?? -100;
        const targetZ = baseZ + scrollY * speed * 0.5;
        mountain.userData.targetZ = targetZ;
        if (progress > 0.7) mountain.position.z = 600000;
        else mountain.position.z = refs.locations?.[i] ?? baseZ;
      });
    ...
    <div ref={containerRef} className="hero-container cosmos-style">
      ...
      <div className="scroll-sections">
        {[...Array(2)].map((_, i) => {
          ...
          const block = subtitles[i + 1];
          return (
            <section key={i} className="content-section">
              <h1 className="hero-title">{titles[i + 1] || "DEFAULT"}</h1>
              <div className="hero-subtitle cosmos-subtitle">
                <p className="subtitle-line">{block.line1}</p>
                <p className="subtitle-line">{block.line2}</p>
              </div>
            </section>
          );
        })}
